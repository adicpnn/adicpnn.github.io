---
title: Bypassing Android client-side protections with Frida
date: 2023-08-01 13:37:00
categories: [Guides,Android]
tags: [android,emulator,frida,anti-root,ssl-pinning]     # TAG names should always be lowercase
---


Long are the days of manually decompiling an apk just to change a few return values.

Due to the fact that Frida only allows 1 script to be executed, I've proceeded to compile a few of my favorite ones into a single <b>monster</b> of a script.

- Make sure proxy is running on the device

- Install the target apk `adb install <TARGET_APK.apk>`

- Download frida server for your architecture [here](https://github.com/frida/frida/releases)

- Install & run `adb push <FRIDA_SERVER_BINARY> /data/local/tmp/frida-server && adb shell chmod 755 /data/local/tmp/frida-server && adb shell /data/local/tmp/frida-server &`

- Get package name of target app `frida-ps -Uai`

- `frida -U -f [APP_ID] -l fridascript-universal.js`

Credits to [coaxial](https://gist.github.com/coaxial), [akabe1](https://codeshare.frida.re/@akabe1/) and [dzonerzy](https://codeshare.frida.re/@dzonerzy/) for creating the scripts.
...
setImmediate(function () {
  Java.perform(function () {
    ...
    var exec2 = Runtime.exec.overload(
      "java.lang.String",
      "[Ljava.lang.String;"
    );
    ...
    if (useProcessManager) {
      ...
      ProcManExec.implementation = function (
        cmd,
        env,
        workdir,
        redirectstderr
      ...
      Java.perform(function () {
        ...
        try {
          ...
          HttpsURLConnection.setSSLSocketFactory.implementation = function (
            ...
            console.log(
              "  --> Bypassing HttpsURLConnection (setSSLSocketFactory)"
            );
        ...
        try {
          ...
          HttpsURLConnection.setHostnameVerifier.implementation = function (
            ...
            console.log(
              "  --> Bypassing HttpsURLConnection (setHostnameVerifier)"
            );
            return; // Do nothing, i.e. don't change the hostname verifier
          };
        ...
        try {
          ...
          TrustManagerImpl.verifyChain.implementation = function (
            ...
            console.log(
              "  --> Bypassing TrustManagerImpl verifyChain: " + host
            );
        ...
        try {
          ...
          trustkit_Activity_2.verify.overload(
            ...
            console.log(
              "  --> Bypassing Trustkit OkHostnameVerifier(cert): " + a
            );
            return true;
          };
        ...
        try {
          ...
          OpenSSLEngineSocketImpl_Activity.verifyCertificateChain.overload(
            ...
            console.log(
              "  --> Bypassing OpenSSLEngineSocketImpl Conscrypt: " + b
            );
          };
        ...
        try {
          ...
          cwac_CertPinManager_Activity.isChainValid.overload(
            ...
            console.log(
              "  --> Bypassing CWAC-Netsecurity CertPinManager: " + a
            );
            return true;
          };
        ...
        try {
          ...
          netty_FingerprintTrustManagerFactory.checkTrusted.implementation =
            function (type, chain) {
              console.log(
                "  --> Bypassing Netty FingerprintTrustManagerFactory"
              );
            };
        ...
        try {
          ...
          Squareup_CertificatePinner_Activity_1.check.overload(
            ...
            console.log(
              "  --> Bypassing Squareup CertificatePinner (cert): " + a
            );
            return;
          };
        ...
        try {
          ...
          Squareup_CertificatePinner_Activity_2.check.overload(
            ...
            console.log(
              "  --> Bypassing Squareup CertificatePinner (list): " + a
            );
            return;
          };
        ...
        try {
          ...
          Squareup_OkHostnameVerifier_Activity_1.verify.overload(
            ...
            console.log(
              "  --> Bypassing Squareup OkHostnameVerifier (cert): " + a
            );
            return true;
          };
        ...
        try {
          ...
          AndroidWebViewClient_Activity_1.onReceivedSslError.overload(
            ...
            console.log(
              "  --> Bypassing Android WebViewClient (SslErrorHandler)"
            );
          };
        ...
        try {
          ...
          AndroidWebViewClient_Activity_2.onReceivedSslError.overload(
            ...
            console.log(
              "  --> Bypassing Android WebViewClient (WebResourceError)"
            );
          };
